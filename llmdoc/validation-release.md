...
- **2026-09-14 · RD 修复回合（BUG-013 + 现行 dist.rs 下 macOS 复跑）**：**macOS 复跑复现既有产物**——现行 `xtask/src/dist.rs`（新增 `host`/`target`/`samePlatformAsBuild`、动态依赖判据 `same_arch_and_os`、`readelf` 全文 + DT_NEEDED 计数）下 `cargo xtask dist --target aarch64-apple-darwin --check-reproducible`：sha256 **`ab693cc3…`**、25 112 080 B 与已录证据**逐字节一致**，两次独立构建同哈希；随后 `cargo xtask smoke` 7 步全过、`smoke-bootstrap` 通过、`cargo xtask check` 7/7、`cargo test --workspace` 558/0/2 → **macOS 半边证据在现行代码下仍有效，无需整轮重验**（§6 的 macOS 行）。**BUG-013 修复已实现（测试侧）**：`crates/server/tests/common/mod.rs` 新增 `settle_after_listening_line()`（300 ms，与 `storage.rs:1817` 同口径），`backup_restore.rs`/`config_cli.rs` 的 `ServeProcess::start` 在读完 `listening on` 后调用；**只加等待、断言/阈值/ignore 均未动**（diff 存档 `artifacts/web-mvp/t22-rd/bug013/rd-fix.diff`），发布二进制不受影响（macOS 侧哈希一致即证）。**Linux 容器取证 BLOCKED（环境，非仓库问题）**：Docker 引擎再次出现 ADR-035 §5 的同一签名（VM 起后 1.5 s 被宿主 `POST /shutdown`，`ExitBadState`），等待/restart/stop+start/CLI start 等非破坏性动作均无效 → **3× 精确用例与 workspace 558/0/2 未执行，本记录不主张 BUG-013 已验证**；证据与所需用户动作见 `artifacts/web-mvp/t22-rd/bug013/docker-engine-blocked.txt`。决策：ADR-037（含"构建环境知识"：`CARGO_SOURCE_*` env 镜像机制实测不生效、索引目录名是哈希敏感输入、冷缓存预取做法）。日志：`artifacts/web-mvp/t22-rd/{macos-rerun,bug013}/**`；实现记录：[implementation §R30](requirements/web-mvp/implementation.md)。§1–§8 合同语义未改动。
